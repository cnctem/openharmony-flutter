 ...
 *
 * Based on android_test.dart originally written by
 * Copyright (C) 2019  Jonah Williams <jonahwilliams@google.com>
 *
 */
...
import 'package:flutter_tools/src/build_system/targets/ohos.dart';
...
void main() {
  ...
  testUsingContext('debug bundle contains expected resources', () async {
    ...
    await const DebugOhosApplication().build(environment);
  ...
  testUsingContext('OhosAot can build provided target platform', () async {
    processManager = FakeProcessManager.empty();
    final Environment environment = Environment.test(
      fileSystem.currentDirectory,
      outputDir: fileSystem.directory('out')..createSync(),
    ...
    const OhosAot ohosAot =
        OhosAot(TargetPlatform.ohos_arm64, BuildMode.release);

    await ohosAot.build(environment);

    expect(processManager, hasNoRemainingExpectations);
  });

  testUsingContext('OhosAot provide code size information.', () async {
    processManager = FakeProcessManager.empty();
    final Environment environment = Environment.test(
      fileSystem.currentDirectory,
      outputDir: fileSystem.directory('out')..createSync(),
  ...
  testUsingContext('kExtraGenSnapshotOptions passes values to gen_snapshot',
    ...
    await const OhosAot(TargetPlatform.ohos_arm64, BuildMode.release)
        .build(environment);
  });
  ...
  testWithoutContext('ohos aot bundle copies so from abi directory', () async {
    final Environment environment = Environment.test(
      fileSystem.currentDirectory,
      outputDir: fileSystem.directory('out')..createSync(),
      defines: <String, String>{
    ...
    const OhosAot ohosAot =
        OhosAot(TargetPlatform.ohos_arm64, BuildMode.release);
    const OhosAotBundle ohosAotBundle = OhosAotBundle(ohosAot);
    ...
    await ohosAotBundle.build(environment);
